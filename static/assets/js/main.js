...
if (nav) {
  ...
  const LogoUrl = themeId === "Inverted"
    ? "/assets/media/favicon/main-inverted.png"
    : "/assets/media/favicon/main.png";
...
window.addEventListener("load", () => {
  ...
  const existing = document.getElementById("global");

  if (existing) {
    existing.href = blobUrl;
  } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setCloak(nameValue, iconUrl) {
    ...
    const finalName = customName || nameValue;
    const finalIcon = customIcon || iconUrl;
...
// Panic Key
document.addEventListener("DOMContentLoaded", () => {
  const eventKey = JSON.parse(localStorage.getItem("eventKey")) || ["Control", "E"];
  const pLink = localStorage.getItem("pLink") || "https://classroom.google.com/";
  let pressedKeys = [];

  document.addEventListener("keydown", event => {
    pressedKeys.push(event.key);
    if (pressedKeys.length > eventKey.length) pressedKeys.shift();
